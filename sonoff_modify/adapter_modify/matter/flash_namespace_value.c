...
#if CONFIG_FLASH_ORIGIN_API    
    partition_info = bk_flash_get_info(BK_PARTITION_MATTER);
#else
    partition_info = bk_flash_partition_get_info(BK_PARTITION_MATTER);
#endif
...
bk_err_t bk_flash_read_matter_factory(uint32_t offset, uint8_t *value, uint32_t length)
{
    ...
    pt = bk_flash_partition_get_info(BK_PARTITION_MATTER_FACTORY);

    return bk_flash_read_bytes(pt->partition_start_addr + offset, value, length);
#endif
    /* sonoff modify end */
}
...
void  bk_erase_all_test(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    dwRtn = bk_erase_all(BK_PARTITION_MATTER);
