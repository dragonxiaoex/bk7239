...
CHIP_ERROR ConfigurationManagerImpl::GetSoftwareVersionString(char * buf, size_t bufSize)
{
    ...
    sprintf(temp, "%lu.0", software_version);
    VerifyOrReturnValue(bufSize >= strlen(temp), CHIP_ERROR_BUFFER_TOO_SMALL);
    strcpy(buf, temp);
    #endif
...
CHIP_ERROR ConfigurationManagerImpl::GetSoftwareVersion(uint32_t & softwareVer)
{
    ...
    softwareVer = SONOFF_MATTER_SOFTWARE_VERSION;
    /* sonoff modify end */
...
void ConfigurationManagerImpl::DoFactoryReset(intptr_t arg)
{
    ...
    bk_erase_all((bk_partition_t)BK_PARTITION_OT_SETTING);
    bk_erase_all((bk_partition_t)BK_PARTITION_MATTER);
