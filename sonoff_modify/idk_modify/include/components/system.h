...
/* sonoff modify start */
/**
 * @brief 读取芯片OTP中的原始MAC.
 *
 * BK7239N固定读取OTP2的OTP_MAC_ADDRESS1项前6字节, 不受运行时BASE MAC覆盖影响.
 * 不回退到其他MAC项、Flash、随机或默认MAC.
 * 调用前需初始化OTP驱动; 失败时清零非NULL输出缓冲区.
 *
 * @param [out] mac - 至少BK_MAC_ADDR_LEN字节的缓冲区.
 * @return BK_OK表示成功, BK_ERR_NULL_PARAM表示空指针, BK_ERR_ZERO_MAC表示全零地址,
 *         BK_ERR_GROUP_MAC表示组播或广播地址, BK_ERR_NOT_SUPPORT表示未启用OTP或芯片不支持,
 *         其他错误码表示OTP读取失败.
 */
