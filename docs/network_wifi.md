...
本文以当前 BK7239N 固件为基线。架构按 C4 的系统、容器、组件逐层说明，使用 Mermaid 表达边界与关系；UML 时序图和状态图补充运行细节。

## 1. 系统上下文与容器边界（C4 L1 / L2）
...
| 调试人员及串口终端 | 发起连接、AP 和扫描命令 | CLI 目前直接调用 Wi-Fi 管理，与通过网络管理接入的业务路径不同 |

从 C4 容器层看，这些能力均部署在同一个**应用固件容器（C/C++、FreeRTOS）**中。`snf_main`、`snf_wifi` 是固件内部任务，不是独立容器；SDK Wi-Fi、NETIF 和 lwIP 是固件内依赖。
