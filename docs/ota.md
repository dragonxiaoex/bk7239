...
HTTP 与 Matter 共用 Sonoff OTA 核心，负责接收数据、顺序写入暂存分区、提交升级并重启。

本篇以当前 `build_tool/config/bk7239n/` 配置、`sonoff/ota/` 实现及项目 SDK 覆盖代码为基线，说明设备内外的职责边界、数据接收、升级提交和重启后安装流程；计划中的渠道打包与解析单独标明。
...
应用固件与 BL2 是不同的部署产物，在重启前后分别执行接收和安装职责。`snf_ota`、协议接入任务及 SDK 库均运行于应用固件中。
