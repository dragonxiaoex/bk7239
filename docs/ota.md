...
本文依据当前 `build_tool/config/bk7239n/` 配置、`sonoff/ota/` 实现及 SDK 覆盖代码，说明构建打包、设备接收、升级提交和重启后安装流程。
...
| 用户的浏览器              | 通过 Wi-Fi/HTTP 向设备上传文件     | 带封装的 OTA 包          |
| Matter OTA Provider | 向设备的 OTA Requestor 提供升级数据 | Matter 头内为带封装的 OTA 包          |
...
flowchart LR
    ...
    subgraph device ["软件系统：Sonoff 设备"]
        ...
        loader["引导固件，按构建配置校验与安装"]
        imageStore[("数据存储：Flash，主镜像、OTA、升级控制区")]
    end
    browser -->|"HTTP 上传"| firmware
    provider -->|"Matter OTA / BDX，经 Thread"| firmware
    firmware -->|"写暂存数据，调用平台升级提交接口"| imageStore
    loader -->|"按构建模式校验、解压并安装"| imageStore
    loader -->|"主镜像验证通过后启动"| firmware
```

应用固件在重启前接收镜像并提交升级，引导固件在重启后校验和安装。`snf_ota`、协议接入任务及 SDK 库均属于应用固件。
...
图示为已有组件，实线表示调用，虚线表示回调。OTA 封装解析由 HTTP/Matter 共用的 OTA 核心完成；文件内容原样写入，平台镜像格式由 SDK/bootloader 处理。
...
| OTA 核心    | [sonoff_ota.c](../sonoff/ota/sonoff_ota.c)、[sonoff_ota.h](../sonoff/ota/sonoff_ota.h)：单会话、顺序/长度检查、按需擦除、状态通知；[sonoff_ota_parse.c](../sonoff/ota/sonoff_ota_parse.c) 负责OTA 元数据、文件选择、解密认证、外层 CRC 和分区容量校验                  |
...
安全构建同时通过 SDK 的 `PACK_RAW_OTA_BIN` 生成 `ota_raw.bin`，其中直接包含 BL2 所需的签名压缩镜像。OTA 外层打包选用该产物；原有 `ota.bin` 仍由 `PACK_OTA_BIN` 生成并保留。Sonoff 解析器和外层打包脚本均不解析或剥离 BK 传输头。
...
```text
HTTP → 带封装的 OTA 包 → OTA 核心
Matter → Matter 头剥离 → 带封装的 OTA 包 → OTA 核心
OTA 核心 → 校验元数据、文件属性和容量 → 按文件名选择 → 按加密类型解密
选中的完整明文文件 → 从 OTA 分区偏移 0 顺序原样写入
接收完整且外层 CRC/GCM 认证通过 → Apply 调用平台提交接口 → 重启后由平台安装
```

带封装的 OTA 包由 24 字节元数据头、N × 76 字节文件属性和文件内容组成，布局与 [解析接口定义](../sonoff/ota/sonoff_ota_parse.h) 和 [打包脚本](../build_tool/ota/pack_ota.py) 对应。外层封装中的整数为大端序；文件内容不作格式解释。
...
| 选中的目标文件    | 加密前的完整文件，不含封装中的 IV 和标签 | 同上，分块累计                           |
...
所选文件的明文长度必须非零且不超过 OTA 分区容量，加密文件扣除 IV 和标签后检查容量。封装解析层不检查镜像 magic、内部版本、目标地址、算法标志或内部 CRC，也不剥离、改写或延迟补写内部头部。发布侧应选用平台提供的可直接写入 OTA 分区的镜像。带封装的 OTA 包可有多个文件，但本模块仅提取当前芯片的一个升级文件，不分发协同固件，也不解析其他芯片的专用内层格式。Matter 自身的头由 Matter SDK 解析器处理。

本项目的线上结构版本统一使用 `1`，由 `SNF_OTA_STRUCT_VERSION` 和打包脚本的 `STRUCT_VERSION` 定义。

现有 `SnfOtaConfig` 零初始化仍表示 `SNF_OTA_FORMAT_RAW`，保持原始镜像直接写入行为；HTTP/Matter 显式使用 `SNF_OTA_FORMAT_PACKAGE`，只接受带封装的 OTA 包；不再根据文件 magic 自动识别裸平台镜像。包模式自带 CRC 校验和按头部加密类型选择的解密流程，入口的 `image_info.check_type`、`image_info.cipher_type` 仍传 `NONE`，实际加密类型以 `SnfOtaMetadata.cipher_type` 为准。
...
sequenceDiagram
    ...
    loop 顺序提交数据块
        ...
        worker->>worker: 解析 OTA 封装头，按需解密并累计完整文件CRC
...
| `VERIFY_SUCCESS` | 包模式下输入完整、头部/明文 CRC 通过，加密包 GCM 认证通过；安全构建的镜像签名由 BL2 验证 |
| `APPLY`          | 正在提交升级确认标志                              |
| `SUCCESS`        | 应用侧升级提交返回成功，随后将请求重启；不是新镜像已安装或业务已正常运行    |

RAW 模式的 `CHECK_SHA256` 分支目前返回不支持；包模式按封装字段执行 CRC/GCM 校验。安全构建的启动镜像哈希与签名由 BL2 验证，普通构建由 bootloader 按 RBL 格式校验。
...
| HTTP / Matter 写入等待 | 分别为 30 / 20 秒，通过 `pdMS_TO_TICKS()` 转换为系统 tick |
| 提交后重启延时            | 2 秒                                           |
...
主机回归在 `build_tool` 目录运行 `python3 tests/ota/test_ota.py`，覆盖头部任意分块、文件选择、CRC 错误、越界、截断、会话重启、加密打包与解密、GCM 篡改拒绝、固定密钥读取、任意文件原样落盘、分区容量边界以及 SECURE 构建配置和 SDK 原产物选择。测试使用实际 OTA 核心、解析器、AES-GCM 适配及 SDK CRC/GCM 实现，队列调度与 Flash 为替身；板端 HTTP/Matter 传输和 BL2 安装仍需实机验证。
