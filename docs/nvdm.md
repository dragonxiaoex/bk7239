...
`project/` 下每个子目录对应一个私有项目，目前为演示项目 `onoff_plug`。公共 [私有入口头文件](../sonoff/private/sonoff_private_device.h)和[产测头文件](../sonoff/private/sonoff_private_factory.h)规定每个项目必须实现的接口；具体 NVDM item 由项目的 `sonoff_private_item.h` 定义。
...
图示描述正常 GPIO 操作和存储路径。HTTP 调用 `snfPlugOnOffSet()`，但其中主动上报 Matter 的调用当前被注释，不能据此保证 HTTP 控制后 Matter 属性立即同步。Matter 属性变化回调使用 `snfPlugOnOffRawSet()`。启动同步位于 [chipinterface.cpp](../project/onoff_plug/matter/src/chipinterface.cpp) 的 `InitServer()`，在属性表初始化完成后读取当前 GPIO；上报接口返回成功只表示任务已提交。
