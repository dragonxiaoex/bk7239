...
本文面向产测人员和产测上位机开发者，按当前 CLI 和 NVDM 实现说明产测进入、生产数据写入、读回校验及返工操作。项目示例采用 `project/onoff_plug`，指令速查见 [at.md](at.md)。
...
| 有效 License，握手超时 | 进入正常业务；当前调试配置保留控制台 UART 接收 | 要进入产测启动分支，需复位并在下一次启动窗口内握手 |
...
当前 `sonoffEntry()` 未调用关闭串口接收的函数，AT 处理入口也未统一限制为仅产测模式可用。产测握手控制的是启动分支，不能据此判断正常模式下 AT 已关闭。
...
```text
发送：AT+FW_VER?\r\n
成功：AT+FW_VER=FWSW-01-SWITCH-BK7239N-v1.1.2\r\n
失败：AT+FW_VER=ERROR\r\n
```
...
AT 应答不包含构建时间戳和 `TEST/FACTORY`；导出文件命名规则见 [编译说明](build.md)。
...
| 现象                           | 建议处理                                                  |
| ---------------------------- | ----------------------------------------------------- |
| 有启动日志，但 AT 无应答 | 核对控制台端口、波特率、CRLF 和命令名；当前调试配置保留串口接收，产测握手超时本身不会关闭 AT |
...
| 内容                        | 实现入口                                                                                                      |
| ------------------------- | --------------------------------------------------------------------------------------------------------- |
| 指令速查 | [at.md](at.md)，License 完整约定见本文第 4 节 |
